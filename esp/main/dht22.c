/*
Based on https://github.com/Andrey-m/DHT22-lib-for-esp-idf/blob/master/src/DHT_c.c
*/
...
DHT22Reading fail(int response)
{
...
int getSignalLevel(int DHTgpio, int usTimeOut, bool state)
{
...
DHT22Reading dht22_read_sync(int DHTgpio)
{
    ...
    uSec = getSignalLevel(DHTgpio, 85, 0);
    // ESP_LOGI(TAG, "Response = %d", uSec);
    if (uSec < 0)
        return fail(DHT_TIMEOUT_ERROR);

    // -- 80us up ------------------------

    uSec = getSignalLevel(DHTgpio, 85, 1);
    // ESP_LOGI(TAG, "Response = %d", uSec);
    if (uSec < 0)
        return fail(DHT_TIMEOUT_ERROR);
    ...
    for (int k = 0; k < 40; k++)
    {
        ...
        uSec = getSignalLevel(DHTgpio, 56, 0);
        if (uSec < 0)
            return fail(DHT_TIMEOUT_ERROR);

        // -- check to see if after >70us rx data is a 0 or a 1

        uSec = getSignalLevel(DHTgpio, 75, 1);
        if (uSec < 0)
            return fail(DHT_TIMEOUT_ERROR);
    ...
    {
        return fail(DHT_CHECKSUM_ERROR);
    }
}